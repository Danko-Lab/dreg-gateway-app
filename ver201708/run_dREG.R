...
cat("4) -------- Merge peaks\n");
system( paste( "bash ", src_path, "/writeBed.bsh", " 0.15 ", file.dreg.pred.gz, sep="") );
...
## load drEG_HD model
load(args[5]);
t <- system.time( dREG_HD( bed_path = file.dreg.peak.gz, bigwig_plus = ps_plus_path, bigwig_minus = ps_minus_path, model=model, ncores = ncores, use_rgtsvm= use_rgtsvm ) )
cat("Running time [User]:", t[1], "[System]:", t[2], "[Elapsed]:", t[3], "\n");
